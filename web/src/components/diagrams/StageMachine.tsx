  ...
  return (
    <div>
        ...
        {STAGE_ORDER.map((s, i) => (
          <div key={s} className="flex items-center gap-1.5">
            <button
              ...
              className={`rounded-[3px] border px-3 py-1.5 font-mono text-[0.8125rem] uppercase tracking-[0.1em] transition-colors duration-150 ${
                stage === s
      ...
      <p className="mt-4 text-[0.9375rem] text-ink-2">
        <span className="font-mono text-[0.8125rem] uppercase tracking-[0.1em] text-ink">
          {stage}
        </span>{' '}
        ...
        {advance && (
          <>
            {' '}Advances to{' '}
            <span className="font-mono text-[0.8125rem] uppercase text-value">{advance.to}</span> on
      ...
      {legal.length === 0 ? (
        <p className="mt-5 font-mono text-[0.875rem] text-ink-3">
        ...
        <ul className="mt-5 flex flex-wrap gap-2">
          {legal.map((id) => {
            ...
            return (
              ...
              >
                <span className="font-mono text-[0.875rem] text-ink">{id}</span>
                <span className="num ml-2 font-mono text-[0.75rem] text-ink-3">
                  cost {COST.get(id)?.toFixed(1)}
                </span>
                {tool && (
                  <span className="ml-2 font-mono text-[0.75rem] text-atk" title={`requires ${tool}`}>
                    genai
                  </span>
                )}
              </li>
