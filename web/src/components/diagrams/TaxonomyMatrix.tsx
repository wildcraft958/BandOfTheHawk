  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full min-w-[52rem] border-collapse text-left">
        ...
        <thead>
          <tr className="border-b border-rule-strong font-mono text-[0.75rem] uppercase tracking-[0.12em] text-ink-3">
          ...
          {simulated.map((v) => (
            <tr key={v.id} className="border-b border-rule align-top">
              <td className="py-3 pr-4">
                ...
                <div className="mt-1 max-w-md text-[0.9375rem] text-ink-3">{v.blurb}</div>
              </td>
              <td className={`py-3 pr-4 font-mono text-[0.8125rem] uppercase ${STAGE_TONE[v.entryStage]}`}>
                {v.entryStage}
              </td>
              <td className="py-3 pr-4 text-[0.9375rem] text-ink-2">
              ...
              <td className="num py-3 pr-4 text-right text-ink">
                {v.heldOut ? (
                  <span className="font-mono text-[0.8125rem] text-holdout">held out</span>
                ) : (
                  (episodes[v.id] ?? '.')
                )}
              </td>
          ...
          {excluded.map((v) => (
            <tr key={v.id} className="border-b border-rule align-top opacity-55">
              <td className="py-3 pr-4">
                ...
                <div className="mt-1 max-w-md text-[0.9375rem] text-ink-3">{v.exclusion}</div>
              </td>
              <td className="py-3 pr-4 font-mono text-[0.8125rem] uppercase text-ink-3">
                {v.entryStage}
              </td>
              <td className="py-3 pr-4 text-[0.9375rem] text-ink-3">{v.capability}</td>
              <td className="py-3 pr-4 text-right font-mono text-[0.8125rem] text-ink-3">.</td>
            </tr>
          ))}
        </tbody>
      </table>
