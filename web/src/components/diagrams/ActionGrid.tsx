...
export function ActionGrid() {
  ...
  return (
    <div>
      <p className="mb-6 max-w-3xl text-[0.875rem] text-ink-2">
      ...
      <div className="grid gap-x-8 gap-y-8 sm:grid-cols-2 lg:grid-cols-4">
        {PHASES.map((phase) => (
          <div key={phase}>
            <h3 className="border-b border-rule-strong pb-2 font-mono text-[0.625rem] uppercase tracking-[0.12em] text-ink-3">
              {PHASE_LABEL[phase]}
            </h3>
              ...
              {ACTIONS.filter((a) => a.phase === phase).map((a) => (
                <li key={a.id}>
                  <div className="flex items-baseline justify-between gap-2">
                    <span
                      className={`font-mono text-[0.75rem] ${a.genaiTool ? 'text-atk' : 'text-ink'}`}
                    >
                      {a.id}
                    </span>
                    <span className="num shrink-0 font-mono text-[0.625rem] text-ink-3">
                      {a.cost.toFixed(1)}
                    </span>
                  </div>
                  <div className="text-[0.75rem] leading-snug text-ink-3">{a.description}</div>
                  {a.genaiTool && (
                    <div className="mt-0.5 font-mono text-[0.625rem] text-atk/80">
                      needs {a.genaiTool}
                    </div>
                  )}
                </li>
