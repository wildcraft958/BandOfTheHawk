...
import textSamplesRaw from './text_samples.json'
...
export const textSamples = textSamplesRaw as unknown as TextSamples
...
export const fixtureFiles: string[] = Object.entries({
  ...
  'text_samples.json': textSamplesRaw,
})
...
if (import.meta.env.DEV) {
  ...
  // The solution document withholds the held-out recall figure, and a number
  // nobody renders still ships if it sits in the bundled JSON, where anyone can
  // read it off the page source. This catches a regenerated fixture that brings
  // it back.
  console.assert(
    !JSON.stringify(coadaptRaw).includes('zero_shot') &&
      !JSON.stringify(coadaptRaw).includes('recall'),
    'a withheld measurement returned to the bundled co-adaptation data',
  )
