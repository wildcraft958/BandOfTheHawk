...
import { PREVALENCE_CAVEAT , BANDS } from '../data/paper'
...
const FEASIBILITY = [
  ...
  },
  {
    ...
    tone: 'def' as const,
    items: [
      'Scoring is one small ensemble per event. Training time is a GPU cost paid offline, not an inference cost.',
  ...
  },
  {
    ...
    items: [
      ...
      'Generalisation to a held-out vertical is not measured, because the held-out action stayed legal in the attacker’s action space.',
...
export function Live() {
  ...
  return (
    <div className="space-y-4 px-4 py-5 sm:px-6">
      <PageHead
        title="Auth stream"
        blurb="Authorisations scored one at a time against the fitted bands, and the graph edit a block performs."
      />
      ...
      <KpiStrip
        items={[
          ...
          {
            label: 'base rate',
            value: pct(PREVALENCE_CAVEAT.measuredAt, 0),
        ...
        aside={
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-panel border border-rule px-2.5 py-1 text-[0.75rem] uppercase tracking-[0.09em] text-ink-2 transition-colors duration-150 hover:border-ink-3 hover:text-ink"
            >
              {running ? (
                <>
      ...
      >
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 border-b border-rule-subtle px-4 py-2.5 text-[0.75rem] text-ink-3">
          <span title="Re-searched against the cost curve at every refit, and jittered per episode, so these are one search's output rather than fixed thresholds.">
            bands, one search
          </span>
          {bands &&
            (
                ...
                <li key={r.id} className={reduced ? undefined : 'rise'}>
                  ...
                  >
                    <ChevronDown
                      ...
                      aria-hidden="true"
                    />
                    <span className="num w-16 shrink-0 text-[0.8125rem] text-ink-3">{r.ts}</span>
                    <span className="num w-20 shrink-0 text-[0.8125rem] text-ink-2">{r.cardId}</span>
                    <span className="num w-20 shrink-0 text-right text-[0.8125rem] text-ink">
                      {r.amount.toFixed(2)}
                    </span>
                    <span className="hidden w-28 shrink-0 text-[0.8125rem] text-ink-3 sm:inline">
                        ...
                        {r.contributions
                          ...
                          .map((c) => (
                            <li key={c.feature} className="flex items-center gap-3">
                              <span className="w-52 shrink-0 truncate text-[0.8125rem] text-ink-2">
                                {c.feature}
                              </span>
                              <span className="num w-32 shrink-0 text-[0.8125rem] text-ink">
                                {c.value}
                              </span>
                              ...
                              <span
                                className={cn(
                                  'num w-14 shrink-0 text-right text-[0.8125rem]',
        ...
        <div className="border-t border-rule px-4 py-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="text-[0.8125rem] text-ink-3">seed {SEED}, deterministic</span>
          </div>
          <Note
            label="what is real here, and what is not"
            className="mt-3 border-t-0 pt-0"
            lede={
              <>
                The score is a transparent linear stand-in whose weights are shown above, not the
                trained ensemble. What this panel demonstrates is the mitigation ladder.
              </>
            }
          >
            <p className="prose-sans max-w-3xl text-[0.9375rem] leading-relaxed text-ink-2">
              <span className="text-ink">Real:</span> the amount distribution (lognormal mu{' '}
              {fixed(fidelity.amount.lognormal_mu, 4)}, sigma{' '}
            ...
            <p className="prose-sans mt-2 max-w-3xl text-[0.9375rem] leading-relaxed text-ink-2">
              <span className="text-ink">Not real:</span> the individual events are sampled, and the
              score is the stand-in noted above rather than the trained XGBoost ensemble. The shape
              of a scored authorisation is the part that has to fit inside an authorisation path,
              and that is what is being shown.
            </p>
          </Note>
        </div>
      </Panel>

      <Panel name="mitigation is graph surgery" tone="atk">
        <EntityGraph />
      </Panel>
              ...
              {group.items.map((item) => (
                <li key={item} className="flex gap-2.5">
                  <span
                    ...
                    aria-hidden="true"
                  />
                  <span className="prose-sans text-[0.9375rem] leading-relaxed text-ink-2">
                    {item}
                  </span>
                </li>
              ))}
      ...
      <Panel name="commercial viability" tone="pass">
        ...
        <p className="prose-sans mt-3 max-w-3xl text-[0.9375rem] text-ink-3">
